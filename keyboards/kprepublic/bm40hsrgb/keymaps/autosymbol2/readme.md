...
## features
  ...
  ### gaming layer  
    ...
    find good keys for shift and space for WASD
  ...
  ### SINGLE_HOLD symbol pseudo layer / AUTOSYMBOL layer
    ...
    layout partly inspired by NEO, AltGr symbols, and shifted number symbols 
  ### SFT_HOLD pseudo layer
    gives the ability to map Shift(SINGLE_HOLD) to any key
    alternative approach to DOUBLE_TAP_HOLD
    comfortable way to map symbols on home row mod keys
    ...
    * unused 3rd funktion  
     ...
     x  bm  
    * tricky case:  
      will only trigger for two handed operation on home row mods
...
### (todo optimize) one hand typing:
        is currenly activated on layer _L3, wich is not onehand
